...
//! `owt-api-types` [`ClientFrame`]/[`ServerFrame`] protocol. Until the server ships,
//! [`WsClient`] encodes the frames it *would* send and returns
//! [`ClientError::Unimplemented`]; [`encode`] is the real wire encoder the follow-up
//! keeps (ADR-0002).
...
impl WsClient {
    ...
    pub async fn connect(&self) -> Result<(), ClientError> {
        let _auth = self.bearer.as_deref();
        Err(ClientError::Unimplemented("WsClient::connect"))
    }

    /// Subscribe to a topic.
    pub async fn subscribe(&self, topic: &str) -> Result<(), ClientError> {
        let _frame = encode(&ClientFrame::Subscribe {
            topic: topic.to_owned(),
        })?;
        Err(ClientError::Unimplemented("WsClient::subscribe"))
    }

    /// Unsubscribe from a topic.
    pub async fn unsubscribe(&self, topic: &str) -> Result<(), ClientError> {
        let _frame = encode(&ClientFrame::Unsubscribe {
            topic: topic.to_owned(),
        })?;
        Err(ClientError::Unimplemented("WsClient::unsubscribe"))
    }

    /// Await the next server frame.
    pub async fn next_frame(&self) -> Result<ServerFrame, ClientError> {
        Err(ClientError::Unimplemented("WsClient::next_frame"))
    }
}
